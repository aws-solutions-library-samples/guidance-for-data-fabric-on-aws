...
import { TaskType, type DataAssetTask } from '../stepFunction/tasks/models';
...
export class GlueCrawlerEventProcessor {
	...
	private async constructLineage(id:string, dataAssetTask:DataAssetTask, event: CrawlerStateChangeEvent, tableName:string): Promise<Partial<RunEvent>>{
		...
		const builder = new OpenLineageBuilder(asset.catalog.domainId, asset.catalog.domainName, asset.execution.hubExecutionArn, ['TODO User']);
		const lineage = builder.setJob(
			{
