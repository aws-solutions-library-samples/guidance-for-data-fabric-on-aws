...
export const privateSubnetIdsParameter = (environment: string) => `/sdf/shared/${environment}/network/privateSubnets`;
export const isolatedSubnetIdsParameter = (environment: string) => `/sdf/shared/${environment}/network/isolatedSubnets`;
export const privateSubnetIdListParameter = (environment: string) => `/sdf/shared/${environment}/network/privateSubnetList`;
	...
	constructor(scope: Construct, id: string, props: NetworkConstructProperties) {
		...
		if (props.userVpcConfig === undefined) {
			...
			new ssm.StringListParameter(this, 'privateSubnetIdListParameter', {
				parameterName: privateSubnetIdListParameter(props.environment),
				description: 'Private subnet IDs used for SDF.',
			...
			new ssm.StringListParameter(this, 'privateSubnetIdListParameter', {
				parameterName: privateSubnetIdListParameter(props.environment),
				description: 'Private subnet IDs used for SDF.',
				stringListValue: props.userVpcConfig.privateSubnetIds
			});
			...
			// internally SDF keeps 3 subnet groups to be compatible with deployments before removing public subnets
